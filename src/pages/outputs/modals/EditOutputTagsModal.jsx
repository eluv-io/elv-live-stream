...
import pillStyles from "@/pages/streams/modals/EditTagsModal.module.css";

const EditOutputTagsModal = observer(({opened, onClose, records=[]}) => {
  ...
  const HandleSave = async() => {
    try {
      ...
      for(const record of records) {
        ...
        await outputStore.UpdateOutputTags({
          outputId: record.slug,
          tags: recordTags
        });
      }
      notifications.show({
  ...
  return (
    ...
    >
      ...
      <TagsInput
        description="Add tags to organize and quickly find outputs. New tags will be applied to all selected items."
        styles={{description: {fontSize: "0.875rem"}}}
        placeholder="Type and press Enter to add a tag"
        data={outputStore.allOutputTags.filter(t => !allCurrentTags.includes(t))}
        value={newTags}
        onChange={setNewTags}
        mb={24}
        clearable
...
export default EditOutputTagsModal;